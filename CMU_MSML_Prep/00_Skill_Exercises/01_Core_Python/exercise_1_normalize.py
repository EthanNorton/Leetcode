...
def normalize(xs):
    ...
    # Hints:
    # 1. Check if xs is empty
    # 2. Calculate mean: sum(xs) / len(xs)
    # 3. Calculate variance: sum((x-mean)**2 for x in xs) / len(xs)
    # 4. Calculate std: variance**0.5
    # 5. Return list comprehension: [(x-mean)/std for x in xs]
...
def normalize_numpy(xs):
    ...
    This is what you'd use in practice, but implement the pure Python version first!
    """
    import numpy as np
    xs_array = np.array(xs)
    mean = np.mean(xs_array)
    std = np.std(xs_array, ddof=0)  # Population std
    if std == 0:
